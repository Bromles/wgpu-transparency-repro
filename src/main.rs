...
impl Renderer {
    fn new(window: Arc<Window>, runtime: Arc<Runtime>) -> Self {
        ...
        let transparent_alpha_mode = surface_capabilities
            .alpha_modes
            .iter()
            .copied()
            .find(|m| *m != CompositeAlphaMode::Opaque)
            .or(Some(CompositeAlphaMode::Auto))
            .expect("Failed to get surface alpha mode");
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if let Self::Loading = self {
            ...
            let window_attributes = WindowAttributes::default()
                .with_title("Wgpu transparent window")
